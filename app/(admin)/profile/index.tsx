...
import { useAdminStats, useAuth } from '../../../hooks';
...
export default function AdminProfile() {
  ...
  const { success: showSuccess, error: showError, confirmDestructive, info } = useAlert();
  const { user, signOut } = useAuth();
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const { isLoading: isUploadingAvatar, saveAvatar, removeAvatar } = useAvatar();
  const { stats: adminStats, refresh: refreshAdminStats } = useAdminStats(); // Get admin stats for Total Orders count
  ...
  const handleRefresh = async () => {
    setRefreshing(true);
    await Promise.all([loadProfile(), refreshAdminStats()]);
    setRefreshing(false);
  };
  ...
  const openWebsite = async () => {
    const url = 'https://kitchen-one-website-vcov.vercel.app/';
    try {
  ...
  const profileActions: ProfileActionItem[] = [
    {
      id: 'dashboard',
      title: 'Dashboard',
      subtitle: 'View admin dashboard and analytics',
      icon: 'dashboard',
        ...
        <ResponsiveView padding="lg">
          ...
          <ResponsiveView style={[styles.profileHeader, { backgroundColor: colors.surface }]}>
            ...
            <ResponsiveView style={styles.userInfo}>
              ...
              <ResponsiveView style={styles.roleContainer} marginTop="sm">
          ...
          <ResponsiveView style={styles.section}>
            ...
            <ResponsiveView style={[styles.infoCard, { 
              ...
              <ResponsiveView style={styles.infoRow}>
                <ResponsiveView style={styles.infoLabel}>
                  <ResponsiveView style={[styles.infoIcon, { backgroundColor: colors.surfaceVariant }]}>
                    <MaterialIcons name="admin-panel-settings" size={20} color={colors.themedText} />
                  </ResponsiveView>
                  <ResponsiveView marginLeft="sm">
                    <ResponsiveText size="md" color={colors.textSecondary}>
                      Role
                ...
                <ResponsiveView style={[styles.statCard, { backgroundColor: colors.background }]}>
                  ...
                  <ResponsiveView style={styles.statContent}>
                    <ResponsiveText size="lg" weight="bold" color={colors.text}>
                      {formatMemberSince(profile?.created_at || new Date().toISOString())}
                    </ResponsiveText>
